...
function expiryStage(expiry){const nowParts=new Intl.DateTimeFormat('en-CA',{timeZone:'Africa/Johannesburg',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()),part=t=>nowParts.find(x=>x.type===t)?.value,today=`${part('year')}-${part('month')}-${part('day')}`,day=v=>Math.floor(Date.UTC(...v.split('-').map((n,i)=>i===1?Number(n)-1:Number(n)))/86400000),days=day(expiry)-day(today);if(days<0)return'expired';if(days<=3)return'critical';if(days<=7)return'urgent';if(days<=14)return'expiring-soon';if(days<=30)return'watch-list';return null}
async function notifyForBatch(batchId,batch){if(!batch||Number(batch.quantity)<=0)return;const stage=expiryStage(batch.expiryDate);if(!stage)return;const members=await db.collection('memberships').where('businessId','==',batch.businessId).where('status','==','active').get(),product=await db.doc(`products/${batch.productId}`).get(),name=product.exists?product.data().name:'A product',write=db.batch(),now=FieldValue.serverTimestamp();for(const member of members.docs){const userId=member.data().userId,id=createHash('sha256').update(`${userId}|${batchId}|${stage}`).digest('hex').slice(0,40);write.set(db.doc(`notifications/${id}`),{businessId:batch.businessId,userId,batchId,productId:batch.productId,type:'expiry-stage',stage,title:`${name}: ${stage.replace('-',' ')}`,message:`Batch ${batch.batchNumber} has reached the ${stage.replace('-',' ')} stage.`,dedupeKey:`${batchId}:${stage}`,readAt:null,createdAt:now,updatedAt:now},{merge:false})}if(!members.empty)await write.commit()}
exports.batchExpiryNotification=onDocumentWritten({document:'batches/{batchId}',region:REGION},async event=>notifyForBatch(event.params.batchId,event.data?.after.exists?event.data.after.data():null));
exports.dailyExpirySummaries=onSchedule({schedule:'0 6 * * *',timeZone:'Africa/Johannesburg',region:REGION},async()=>{const batches=await db.collection('batches').where('quantity','>',0).get();for(const batch of batches.docs)await notifyForBatch(batch.id,batch.data())});

async function assertBusinessMember(request,businessId,manager=false){if(!request.auth)throw new HttpsError('unauthenticated','Sign in required.');const member=await db.doc(`memberships/${request.auth.uid}_${businessId}`).get();if(!member.exists||member.data().status!=='active'||(manager&&!['owner','manager'].includes(member.data().role)))throw new HttpsError('permission-denied','Your role cannot perform this action.');const accessDoc=await db.doc(`businessAccessControls/${businessId}`).get();if(accessDoc.data()?.accessStatus!=='active')throw new HttpsError('failed-precondition','This business is currently read-only.');return member.data()}
exports.recordStockAction=onCall({region:REGION,enforceAppCheck:true},async request=>{const d=request.data||{},businessId=cleanText(d.businessId,128),batchId=cleanText(d.batchId,128),type=cleanText(d.type,30),quantity=Number(d.quantity),allowed=['discount','transfer','return','donate','internal-use','sold','disposed','assign'];if(!businessId||!batchId||!allowed.includes(type)||!Number.isFinite(quantity)||quantity<=0)throw new HttpsError('invalid-argument','Valid action, batch and quantity are required.');await assertBusinessMember(request,businessId,type==='discount');const batchRef=db.doc(`batches/${batchId}`),batch=await batchRef.get();if(!batch.exists||batch.data().businessId!==businessId||quantity>batch.data().quantity)throw new HttpsError('failed-precondition','The batch or quantity is invalid.');const stage=expiryStage(batch.data().expiryDate);if(stage==='expired'&&['sold','discount'].includes(type))throw new HttpsError('failed-precondition','Expired products cannot be sold or discounted.');const actionRef=db.collection('stockActions').doc(),now=FieldValue.serverTimestamp(),write=db.batch(),status=type==='discount'?'pending-approval':'completed';write.create(actionRef,{businessId,productId:batch.data().productId,batchId,type,quantity,employeeId:cleanText(d.employeeId,128),notes:cleanText(d.notes,1000),valueSaved:Number(d.valueSaved)||0,valueLost:Number(d.valueLost)||0,status,createdBy:request.auth.uid,createdAt:now,updatedAt:now});if(['return','donate','internal-use','sold','disposed'].includes(type))write.update(batchRef,{quantity:batch.data().quantity-quantity,updatedAt:now});if(type==='discount'){const percent=Math.max(1,Math.min(Number(d.discountPercent)||0,90)),originalPrice=Number(batch.data().sellingPrice)||0,newPrice=Math.round(originalPrice*(1-percent/100)*100)/100;write.create(db.collection('promotions').doc(),{businessId,productId:batch.data().productId,batchId,actionId:actionRef.id,discountPercent:percent,originalPrice,newPrice,quantity,expectedRevenue:newPrice*quantity,potentialLossPrevented:newPrice*quantity,endDate:batch.data().expiryDate,status:'pending-approval',createdBy:request.auth.uid,createdAt:now,updatedAt:now})}write.create(db.collection('auditLogs').doc(),{businessId,actorUid:request.auth.uid,action:`stock.${type}`,targetId:batchId,details:{quantity},createdAt:now,updatedAt:now});await write.commit();return{ok:true,actionId:actionRef.id}});
exports.approveDiscount=onCall({region:REGION,enforceAppCheck:true},async request=>{const id=cleanText(request.data?.promotionId,128),ref=db.doc(`promotions/${id}`),snap=await ref.get();if(!snap.exists)throw new HttpsError('not-found','Promotion not found.');const data=snap.data();await assertBusinessMember(request,data.businessId,true);if(data.status!=='pending-approval')throw new HttpsError('failed-precondition','This promotion is no longer pending.');const now=FieldValue.serverTimestamp(),write=db.batch();write.update(ref,{status:'approved',approvedBy:request.auth.uid,approvedAt:now,updatedAt:now});write.create(db.collection('auditLogs').doc(),{businessId:data.businessId,actorUid:request.auth.uid,action:'discount.approved',targetId:id,createdAt:now,updatedAt:now});await write.commit();return{ok:true}});
exports.setBusinessMember=onCall({region:REGION,enforceAppCheck:true},async request=>{const businessId=cleanText(request.data?.businessId,128),email=cleanText(request.data?.email,200).toLowerCase(),role=cleanText(request.data?.role,20);if(!['manager','employee'].includes(role)||!email)throw new HttpsError('invalid-argument','Email and a permitted role are required.');const actor=await assertBusinessMember(request,businessId,true);if(actor.role!=='owner')throw new HttpsError('permission-denied','Only the business owner may manage roles.');let target;try{target=await getAuth().getUserByEmail(email)}catch{throw new HttpsError('not-found','That person must register and verify their StockGuard email first.')}const now=FieldValue.serverTimestamp(),ref=db.doc(`memberships/${target.uid}_${businessId}`),write=db.batch();write.set(ref,{businessId,userId:target.uid,email,displayName:target.displayName||email,role,status:'active',createdAt:now,updatedAt:now},{merge:true});write.create(db.collection('auditLogs').doc(),{businessId,actorUid:request.auth.uid,action:'membership.changed',targetId:target.uid,details:{role,email},createdAt:now,updatedAt:now});await write.commit();return{ok:true}});
exports.disableBusinessMember=onCall({region:REGION,enforceAppCheck:true},async request=>{const businessId=cleanText(request.data?.businessId,128),userId=cleanText(request.data?.userId,128);const actor=await assertBusinessMember(request,businessId,true);if(actor.role!=='owner'||userId===request.auth.uid)throw new HttpsError('permission-denied','Only the owner may disable another member.');const ref=db.doc(`memberships/${userId}_${businessId}`),snap=await ref.get();if(!snap.exists)throw new HttpsError('not-found','Membership not found.');const now=FieldValue.serverTimestamp(),write=db.batch();write.update(ref,{status:'disabled',updatedAt:now});write.create(db.collection('auditLogs').doc(),{businessId,actorUid:request.auth.uid,action:'membership.disabled',targetId:userId,createdAt:now,updatedAt:now});await write.commit();return{ok:true}});
